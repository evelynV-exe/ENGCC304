...
int main()
{
    ...
    for (i = 0; i < rows2; i++) {
        ...
        scanf("%d", &Matrix2[i][j]); //typo fixed
        }
    }

    printf("\nMatrix1 : \n"); //added semi-colon
    for (i = 0; i < rows1; i++) {
        for (j = 0; j < cols1; j++) { //typo fixed
    ...
    rows3 = cols1; //typo fixed
    cols3 = rows1;

    for (i = 0; i < rows3; i++) { //typo fixed
        for (j = 0; j < cols3; j++) {
    ...
    for (i = 0; i < rows3; i++) { //typo fixed
        for (j = 0; j < cols3; j++) {
        printf("%d ", Matrix3[i][j]); //typo fixed + semi-colon
        }
        printf("\n"); //typo fixed + semi-colon
    }
